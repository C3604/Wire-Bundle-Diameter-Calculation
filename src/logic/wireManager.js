...
export function saveCustomWireLibraries(customWires) {
    ...
    localStorage.setItem(STORAGE_KEYS.CUSTOM_LIBRARIES, JSON.stringify(customWires));
}
...
export function getEffectiveStandardWires() {
    const standardMap = new Map(standardWiresData.map(wire => [String(wire.gauge).trim(), { ...wire }]));
...
export function getSimulationParameters() {
    const storedParamsString = localStorage.getItem(STORAGE_KEYS.SIMULATION_PARAMETERS);
...
export function saveSimulationParameters(params) {
    ...
    localStorage.setItem(STORAGE_KEYS.SIMULATION_PARAMETERS, JSON.stringify(params));
}
