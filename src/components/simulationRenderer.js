...
export function drawCirclesOnCanvas(ctx, canvas, circlesData, options = {}) {
    ...
    if (!circlesData || circlesData.length < 1 || !circlesData[0]) { // 至少需要外层容器数据
        ctx.fillStyle = '#6c757d';
        ctx.textAlign = 'center';
        ctx.font = '14px sans-serif';
        if (canvas.width > 0 && canvas.height > 0) { // 仅在画布有效时绘制文本
            ctx.fillText('无有效的模拟数据用于绘制', canvas.width / 2, canvas.height / 2);
        }
        return;
    }
    ...
    const scale = Math.min(canvas.width - 2 * CANVAS_PADDING, canvas.height - 2 * CANVAS_PADDING) / (2 * outerContainerRadius);
    ...
    for (let i = 2; i < circlesData.length; i++) {
        ...
        // 查找匹配的颜色
        // 使用小的容差来比较浮点数
        const colorEntry = diameterColorMap.find(entry => Math.abs(entry.diameter - circleDiameter) < 1e-6);
        if (colorEntry) {
